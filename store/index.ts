...
export const useStore = create<AppState>((set, get) => ({
    ...
    set((state) => {
      ...
      if (existingCompletion) {
        newCompletions = habit.completions.map((c) =>
          c.date.split('T')[0] === date.split('T')[0]
  ...
  getActivityForDate: (date) => {
    ...
    const completedTasks = tasks.filter(
      (task) =>
        task.is_completed && task.completed_at?.split('T')[0] === dateStr
    ).length;
  ...
  initializeData: () => {
    ...
    set((state) => {
      console.log('Setting initial tasks:', mockData.tasks);
      console.log('Setting initial notes:', mockData.notes);
      console.log('Setting initial habits:', mockData.habits);
      return {
        ...state,
